...
const Analitcs = ({ data }) => {
    ...
    const { data: statisticData } = useGetOneQuery({
        key: "getAllHistoryStatistic",
        url: userId ? `/api/v1/submissions/statistics/user/${userId}` : null,
        enabled: !!userId
    })
    const easyHeight = useMemo(() => {
        if (!data?.data?.by_difficulty?.easy || !statisticData?.difficulty_distribution?.easy) return 0;
        const totalEasy = data?.data?.by_difficulty?.easy;
        const solvedEasy = statisticData?.difficulty_distribution?.easy;
        return ((totalEasy - solvedEasy) * 206) / totalEasy;
    }, [data?.data?.by_difficulty?.easy, statisticData?.difficulty_distribution?.easy]);

    const mediumHeight = useMemo(() => {
        if (!data?.data?.by_difficulty?.medium || !statisticData?.difficulty_distribution?.medium) return 0;
        const totalMedium = data?.data?.by_difficulty?.medium;
        const solvedMedium = statisticData?.difficulty_distribution?.medium;
        return ((totalMedium - solvedMedium) * 206) / totalMedium;
    }, [data?.data?.by_difficulty?.medium, statisticData?.difficulty_distribution?.medium]);

    const hardHeight = useMemo(() => {
        if (!data?.data?.by_difficulty?.hard || !statisticData?.difficulty_distribution?.hard) return 0;
        const totalHard = data?.data?.by_difficulty?.hard;
        const solvedHard = statisticData?.difficulty_distribution?.hard;
        return ((totalHard - solvedHard) * 206) / totalHard;
    }, [data?.data?.by_difficulty?.hard, statisticData?.difficulty_distribution?.hard]);  
                ...
                <Flex mt={'36px'} justifyContent={'space-between'}>
                    <Flex flexDirection={'column'}>
                        ...
                        <Box
                            ...
                            bg={'#52A28A'} {...css.lists}>{statisticData?.difficulty_distribution?.easy}</Box>
                        <Heading color={'#52A28A'} {...css.name}>Easy</Heading>
                    </Flex>
                    <Flex flexDirection={'column'}>
                        <Box height={`${206 - mediumHeight}px`} {...css.list}>{data?.data?.by_difficulty?.medium}</Box>
                        <Box height={`${mediumHeight}px`} bg={'#FFBF1E'} {...css.lists}>{statisticData?.difficulty_distribution?.medium}</Box>
                        <Heading color={'#FFBF1E'} {...css.name}>Medium</Heading>
                    </Flex>
                    <Flex flexDirection={'column'}>
                        <Box height={`${206 - hardHeight}px`} {...css.list}>{data?.data?.by_difficulty?.hard}</Box>
                        <Box height={`${hardHeight}px`} bg={'#FF6063'} {...css.lists}>{statisticData?.difficulty_distribution?.hard}</Box>
